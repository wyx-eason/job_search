...
test("岗位写入幂等且保留来源", () => {
  ...
  const jobs = fixture();
  assert.deepEqual(store.upsertJobs(jobs), { added: 1, updated: 0, blockedSkipped: 0 });
  assert.deepEqual(store.upsertJobs(jobs), { added: 0, updated: 1, blockedSkipped: 0 });
...
test("删除公司会移除全部岗位并屏蔽，屏蔽后不再重新入库", () => {
  ...
  assert.deepEqual(store.upsertJobs(jobs), { added: 0, updated: 0, blockedSkipped: 1 });
  assert.equal(store.listJobs().length, 0);
...
test("屏蔽简称可拦截完整公司名", () => {
  ...
  assert.deepEqual(store.upsertJobs([job]), { added: 0, updated: 0, blockedSkipped: 1 });
  assert.equal(store.listJobs().length, 0);
  store.close();
});
