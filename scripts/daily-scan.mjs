...
export async function runDailyScan({ inputs = [], preferences, store, now = new Date() }) {
  const failures = []; const collected = [];
  ...
  return { pools, failures, persisted, sourceSummary: `读取 ${inputs.length} 个来源，标准化 ${jobs.length} 条岗位` };
}
...
if (path.resolve(process.argv[1] || "") === path.resolve(fileURLToPath(import.meta.url))) {
  ...
  const result = await runDailyScan({ inputs: input ? [path.resolve(input)] : [], preferences, store });
