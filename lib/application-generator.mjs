...
import { buildClassicResumeHtml, buildResumeFromBaseTemplate, splitEntry } from "./resume-classic.mjs";
...
const PHOTO_PATH = "E:/job_search/头像照片/微信图片_20260815115120.jpg";

async function htmlToPdf(html, applyConfigPath, htmlFilePath = null) {
  ...
  try {
    ...
    if (htmlFilePath) {
      await page.goto("file:///" + path.resolve(htmlFilePath).replace(/\\/g, "/"), { waitUntil: "networkidle" });
    } else {
...
export async function generateApplicationPackage({
  ...
  let jdSource = "岗位库";
  const isThin = effectiveJd.length < 80 || !/(岗位职责|任职要求|职位描述|工作职责|岗位描述|岗位要求)/.test(effectiveJd);
  if (isThin && enrichJd) {
    try {
  ...
  const selection = selectBaseResumeFacts(allFacts, effectiveJd, base);
  ...
  if (sectionsOverrides?.internship) {
    sections.internship = (sections.internship || []).map((item) => {
      const head = splitEntry(item).head || item;
      const rule = sectionsOverrides.internship.find((o) => String(head).includes(o.match));
      return rule ? rule.replace : item;
    });
  }
  ...
  if (fs.existsSync(PHOTO_PATH)) {
    fs.copyFileSync(PHOTO_PATH, path.join(packagePath, "photo.jpg"));
    photoSrc = "photo.jpg";
  }
  const tplBase = selection.chosenBase || (base !== "auto" ? base : "ai");
  const html = buildResumeFromBaseTemplate({ candidate, sections, keywords, job: normalizedJob, base: tplBase, photoSrc });
  ...
  let pdf = null;
  try {
    pdf = await htmlToPdf(html, applyConfigPath, path.join(packagePath, "resume.html"));
  ...
  const files = ["job-description.md", "match-report.md", "resume.html", "resume.pdf", "form-answers.json", "generation-audit.json"];
  if (photoSrc) files.splice(files.indexOf("resume.html"), 0, "photo.jpg");
  return {
    packagePath,
