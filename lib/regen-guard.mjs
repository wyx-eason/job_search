...
export function shouldAutoRegenerate({ lastRegen, jdText, url, requireMarkers = true, force = false, detailOk = null }) {
  const text = String(jdText || "").trim();
  if (requireMarkers) {
    const ok = detailOk !== null ? detailOk : /(岗位职责|任职要求|职位描述|工作职责|岗位要求|工作内容|职位信息|岗位介绍|岗位描述)/.test(text);
    if (!ok) return { ok: false, reason: "当前页面不是具体岗位详情，未重新生成" };
  }
