...
export function sanitizeEntryHead(newTitle, originalHead) {
  const raw = String(newTitle || "").trim().replace(/[。]$/, "");
  // LLM 可能把“公司｜角色｜技术栈｜时间”整条链放进标题：
  // 取第一个分隔符前的公司/地点部分，再从整条里取日期，重组为“公司/地点 时间”
  const sep = raw.search(/[｜|]/);
  const companyPart = (sep > 0 ? raw.slice(0, sep) : raw.replace(new RegExp(DATE_RE.source, "g"), "")).trim();
  const dateMatch = raw.match(DATE_RE);
  if (!dateMatch) return originalHead;
  const t = `${companyPart} ${dateMatch[1].trim()}`.replace(/\s+/g, " ").trim();
...
const BASE_TEMPLATE_FILES = {
  ai: path.resolve("E:/job_search/profile/王奕迅_AI应用_基类.html"),
  adas: path.resolve("E:/job_search/profile/王奕迅_ADAS_基类_2026.html"),
  softdev: path.resolve("E:/job_search/profile/王奕迅_软开C++_2026.html"),
  agent: path.resolve("E:/job_search/profile/王奕迅_Agent_基类_2026.html")
};

function buildSectionBlocks(optimized, keywords) {
  const blocks = [];
  for (const key of ["education", "internship", "project", "skills", "honors"]) {
    ...
    blocks.push({ key, title: SECTION_TITLES[key], inner });
  }
  return blocks;
}
...
export function buildClassicResumeHtml({ candidate, sections, keywords, job }) {
  ...
  const blocks = buildSectionBlocks(optimized, keywords).map((b) => `<h2>${b.title}</h2>${b.inner}`).join("\n");
  return `<!doctype html>
<html lang="zh-CN">
<head>
<meta charset="utf-8">
...
export function buildResumeFromBaseTemplate({ candidate, sections, keywords, job, base = "ai", photoSrc = null }) {
  const file = BASE_TEMPLATE_FILES[base] || BASE_TEMPLATE_FILES.ai;
  let tpl;
  try {
    tpl = fs.readFileSync(file, "utf8");
  } catch {
    console.warn(`[简历] 基类模板不存在（base=${base}）：${file}，已回退到经典模板`);
    return buildClassicResumeHtml({ candidate, sections, keywords, job });
  }
  const optimized = optimizeForJob(sections, keywords);
  const blocks = buildSectionBlocks(optimized, keywords);
  for (const block of blocks) {
    const startMark = `<h2>${block.title}</h2>`;
    const startIdx = tpl.indexOf(startMark);
    if (startIdx === -1) continue;
    const afterStart = startIdx + startMark.length;
    const nextIdx = tpl.indexOf("<h2>", afterStart);
    const endIdx = nextIdx === -1 ? tpl.indexOf("</body>") : nextIdx;
    tpl = `${tpl.slice(0, afterStart)}\n${block.inner}${tpl.slice(endIdx)}`;
  }
  if (photoSrc) {
    // 基类模板的 img 为 `<img class="photo" src="...">`（class 在 src 之前），
    // 因此只替换 src 值、保留其它属性，避免照片丢失样式。
    tpl = tpl.replace(/(<img[^>]*?\ssrc=")[^"]*(")/, `$1${esc(photoSrc)}$2`);
  } else {
    tpl = tpl
      .replace(/<img[^>]*class="photo"[^>]*>/g, "")
      .replace(/<div class="photo"[\s\S]*?<\/div>/, "");
  }
  return tpl;
}
