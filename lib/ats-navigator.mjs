...
export function looksLikeJobDetailText(text) {
  return /(岗位职责|职位描述|工作职责|任职要求|岗位要求|工作内容|职位信息|岗位介绍|岗位描述)/.test(String(text || ""));
}

export function isJobDetailPageText(text) {
  const t = String(text || "");
  if (!looksLikeJobDetailText(t)) return false;
  const lines = t.split("\n").map((s) => s.trim()).filter(Boolean);
  // 页面里必须存在“像岗位标题”的行（左列表+右详情的混合页也能识别），
  // 纯列表/筛选页、或以句号结尾的描述句则拒绝
  return lines.some((line) => {
    if (line.length < 4 || line.length > 40) return false;
    if (/^(在招\s*\d+\s*人|共\s*\d+\s*个|全部职位|热招职位|职位列表|筛选|清空|登录|注册)/.test(line)) return false;
    if (/[。！？]$/.test(line)) return false;
    return /工程师|设计师|算法|开发|产品经理|产品|运营|测试|研发|专员|经理|校招|秋招|届|项目管理|策划|美术|市场|用户体验|用户研究|交互|视觉|3D|客户端|服务端|引擎|运维|安全|数据|研究|策略|发行|商务|人力|财务|法务|品牌|公关|AI|PM|程序|技术|管理|培训生|游戏/.test(line);
  });
}
