...
export function looksLikeJobDetailText(text) {
  return /(岗位职责|职位描述|工作职责|任职要求|岗位要求|工作内容|职位信息|岗位介绍)/.test(String(text || ""));
}

export function isJobDetailPageText(text) {
  const t = String(text || "");
  if (!looksLikeJobDetailText(t)) return false;
  // 列表页常把“在招N人/全部职位/共N个”放在 JD 之前，真实岗位详情页不会有
  const beforeJd = t.slice(0, Math.max(0, t.search(/(岗位职责|职位描述|工作职责|任职要求|职位信息|岗位介绍)/)));
  if (/(在招\s*\d+\s*人|共\s*\d+\s*个(?:职位|岗位)|全部职位|热招职位|职位列表)/.test(beforeJd)) return false;
  return true;
}
