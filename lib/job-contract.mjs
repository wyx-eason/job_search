...
export function classifyEligibility(job, now = new Date()) {
  const text = `${job.title_raw} ${job.recruitment_batch} ${job.employment_type} ${job.jd_text}`;
  if (job.deadline && new Date(job.deadline) < now) return { status: "excluded", reason: "申请截止日期已过" };
  if (EXCLUDED_WORDS.test(text) || /实习|intern/i.test(job.employment_type)) return { status: "excluded", reason: "实习或社会招聘岗位" };
  const explicitCampus = job.campus_year === 2027 || CAMPUS_WORDS.test(text);
  if (explicitCampus && !/(实习|intern)/i.test(text)) return { status: "eligible", reason: "识别到 2027 届正式校招或提前批" };
