...
export function extractJdFromPageText(text) {
  ...
  const dutyIndex = joined.search(/(岗位职责|职位描述|工作职责|工作内容|岗位描述)/);
  if (dutyIndex < 0) {
...
export function extractJobTitleFromPageText(text) {
  ...
  const jdIndex = lines.findIndex((line) => /岗位职责|职位描述|工作职责|工作内容|岗位描述|岗位要求|职位信息|岗位介绍/.test(line));
  if (jdIndex > 0) {
    const jobLike = /(工程师|算法|开发|设计师|产品经理|产品|运营|测试|研发|专员|经理|校招|秋招|项目管理|策划|美术|市场|用户体验|用户研究|交互|视觉|3D|客户端|服务端|引擎|运维|安全|数据|研究|策略|发行|商务|人力|财务|法务|品牌|公关|游戏|AI|PM|程序|技术|管理|培训生)/i;
    const isMetaLine = (line) =>
      /^[|｜\s\-–—]*$/.test(line) ||
      /^\d{4}[-/.年]\d{1,2}([-/.月]\d{1,2})?日?$/.test(line) ||
      /^\d+$/.test(line) ||
      /^\d+\s*人$/.test(line) ||
      /首页|登录|注册|职位列表|招聘动态|收藏|分享|投递|申请|职位|招聘|关于|联系|公司名称|企业名称|单位名称|姓名|手机|邮箱|学历|专业|毕业|推荐码|简历|发布|日期|时间|是否|外推|教育背景|实习经历|项目经历|技术技能|荣誉奖项|在招|校园招聘|校招面试|工作地点|招聘类型|投递方式|职位信息/.test(line) ||
      /^[^|｜]*(?:、|,)[^|｜]*(?:、|,)[^|｜]*$/.test(line);
    for (let i = 0; i < jdIndex; i++) {
      const line = lines[i];
      if (line.length < 3 || line.length > 60) continue;
      if (isMetaLine(line)) continue;
      if (jobLike.test(line)) return cleanTitle(cutAtLabels(line));
    }
  }
  return "";
}
