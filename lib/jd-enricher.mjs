...
export function extractJdFromPageText(text) {
  ...
  const dutyIndex = joined.search(/(岗位职责|职位描述|工作职责|工作内容)/);
  if (dutyIndex < 0) {
...
export function extractJobTitleFromPageText(text) {
  ...
  if (jdIndex > 0) {
    const jobLike = /(工程师|算法|开发|设计师|产品|运营|测试|研发|专员|经理|校招|秋招)/;
    for (let i = jdIndex - 1; i >= Math.max(0, jdIndex - 8); i--) {
      const line = lines[i];
      if (
        line.length >= 3 &&
        line.length <= 40 &&
        !/首页|登录|分享|收藏|投递|申请|职位|招聘|关于|联系|公司名称|企业名称|单位名称|姓名|手机|邮箱|学历|专业|毕业|推荐码|简历|发布|日期|时间|是否|外推|教育背景|实习经历|项目经历|技术技能|荣誉奖项|在招|^\d/.test(line)
      ) {
