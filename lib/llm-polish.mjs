...
import { splitEntry } from "./resume-classic.mjs";
...
export function buildPolishPrompt({ jdText, sections, jobTitle = "", userFeedback = "" }) {
  ...
  const parts = [
    ...
    "4. 每条标题行（公司/时间/角色/技术栈）不要改动，只改写「要点」部分；",
    ...
    brief("skills", "技术技能"),
    "",
    '输出 JSON 格式（只输出 JSON）：\n{"internship":[["条目1改写后的要点1","要点2"],["条目2要点"]],"project":[["条目1要点"]],"skills":["改写后的技能要点..."]}\n注意：internship 与 project 是数组的数组，顺序与输入条目一一对应；skills 是字符串数组。'
  ];
    ...
    try {
      ...
      for (const key of ["internship", "project"]) {
        ...
        next[key] = original.map((item, i) => {
          const parts = splitEntry(item);
          const bullets = Array.isArray(lists[i])
            ? lists[i].map((s) => String(s).trim().replace(/[。]$/, "")).filter(Boolean).slice(0, 8)
            : [];
          if (!bullets.length) return item;
          if (parts.date) {
            // 结构化条目：保留公司行/角色/技术栈，只替换要点
            return (
              [parts.head, parts.roleLine, parts.techLine, ...bullets]
                .filter(Boolean)
