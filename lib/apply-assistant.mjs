...
export async function autofillApplyForm(page, candidate, options = {}) {
  ...
  if (options.resumePdfPath) {
    result.uploadedResume = await uploadResumeToForm(page, options.resumePdfPath, { isCurrent: options.isUploadCurrent });
...
export function startFormWatcher({ page, candidate, resumePdfPath, formValues = {}, onFieldsChange, onJobSelection, onJobDetail, onApplyForm, pollIntervalMs = 3000, maxPolls = null, jobMap = null }) {
  ...
  const handleCommit = async (commit) => {
    ...
    const validDetail = isJobDetailUrl(commit.url)
      ...
      || (Boolean(commit.jobTitle) && /(岗位职责|职位描述|工作职责|任职要求|岗位要求|工作内容|职位信息|岗位介绍)/.test(commit.pageText));
  ...
  const timer = setInterval(async () => {
    ...
    try {
      ...
      const committed = committedJobs.splice(0).sort((a, b) =>
        (a.clickedAt || 0) - (b.clickedAt || 0)
        || (a.receiveSequence || 0) - (b.receiveSequence || 0)
      ).at(-1);
      if (committed) {
        await handleCommit(committed);
      }
      ...
      const result = await autofillApplyForm(formPage, candidate, {
...
export async function runApplyAssistant({ applyUrl, config, job = {}, headless = false, maxWaitMs = 30000, resumePdfPath = null, onFieldsChange, onJobSelection, onJobDetail, onApplyForm }) {
