...
import { navigateToApply, isJobDetailUrl, cleanJobTitleForFileName } from "./ats-navigator.mjs";
...
export async function autofillApplyForm(page, candidate, options = {}) {
  ...
  const result = { filled: [], skippedSensitive: [], skippedUnknown: [], submitted: false };
...
export function startFormWatcher({ page, candidate, resumePdfPath, onFieldsChange, onJobDetail, onApplyForm, pollIntervalMs = 3000, maxPolls = null }) {
  ...
  const timer = setInterval(async () => {
    ...
    try {
      const url = page.url();
      if (isJobDetailUrl(url) && url !== lastDetailUrl) {
        lastDetailUrl = url;
        onJobDetail?.({ url });
      }
