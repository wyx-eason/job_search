  ...
  await page
    .evaluate(() => {
      ...
      const markJobSelected = (e) => {
        // 用户真实点击了“像岗位卡片”的元素才算选中岗位；筛选/搜索等点击不算
        if (!e.isTrusted) return;
        // 用 composedPath 穿透 shadow DOM，取点击路径上任一含岗位标题的元素
...
export function extractJobsFromApiPayload(payload) {
  ...
  for (const item of items) {
    const title = String(item.JobAdName || item.JobName || item.Title || "").trim();
    if (!title) continue;
    const duty = String(item.Duty || item.JobDescription || item.Description || "").trim();
    const require = String(item.Require || item.Requirement || "").trim();
    jobs.push({ title, jd: [duty, require].filter(Boolean).join("\n"), id: String(item.Id || item.JobAdId || "") });
  }
  return jobs;
}
...
export function startFormWatcher({ page, candidate, resumePdfPath, onFieldsChange, onJobDetail, onApplyForm, pollIntervalMs = 3000, maxPolls = null, jobMap = null }) {
  ensureUserTouchTracking(page);
  let polls = 0;
  let lastFieldsJson = "";
  let lastDetailUrl = "";
  let lastDetailText = "";
  let sawUserSelected = false;
  const timer = setInterval(async () => {
    ...
    try {
      ...
      for (const candidatePage of pages) {
        ...
        const pageText = await getPageInnerText(candidatePage);
        const userSelected = await candidatePage.evaluate(() => window.__applyJobSelected === true).catch(() => false);
        if (userSelected && !sawUserSelected) {
          const clickedTitle = await candidatePage.evaluate(() => window.__applySelectedJobTitle || "").catch(() => "");
          const matched = jobMap ? jobMap.get(normalizeJobTitle(clickedTitle)) : null;
          console.log(`[投递会话] 检测到用户点击岗位卡片 | 点击岗位=${clickedTitle}${matched ? " | 已匹配接口JD" : " | 未匹配到接口JD"}`);
          if (matched) {
            console.log(`[投递会话] 使用接口岗位数据 | 岗位=${matched.title} | jdLen=${matched.jd.length}`);
          }
        }
        // 混合列表页（如 vivo）：必须用户点击过岗位卡片才按当前内容生成；ATS 详情 URL 不需要
        const isDetail = isJobDetailUrl(url) || (isJobDetailPageText(pageText) && userSelected);
        // 对比全文（含 JD 区域），避免页面尾部固定内容导致换岗位检测不到
        const textKey = pageText.replace(/\s+/g, "");
        const contentChanged = url !== lastDetailUrl || textKey !== lastDetailText;
        const selectionHappened = userSelected && !sawUserSelected;
        if (isDetail && (contentChanged || selectionHappened)) {
          lastDetailUrl = url;
          lastDetailText = textKey;
          if (userSelected) sawUserSelected = true;
          detailPageForForm = candidatePage;
          const clickedTitle = await candidatePage.evaluate(() => window.__applySelectedJobTitle || "").catch(() => "");
          const matched = jobMap ? jobMap.get(normalizeJobTitle(clickedTitle)) : null;
          onJobDetail?.({ url, page: candidatePage, jobTitle: clickedTitle || undefined, jdText: matched?.jd || undefined });
          break;
        }
      }
      // 在最新岗位/表单标签页上自动填表，避免用户切到新标签页后找不到表单
      const formPage = detailPageForForm || page;
      const result = await autofillApplyForm(formPage, candidate, { resumePdfPath });
      if (looksLikeApplyForm(result.formFields || [])) onApplyForm?.({ fields: result.formFields || [], page: formPage });
...
export async function runApplyAssistant({ applyUrl, config, job = {}, headless = false, maxWaitMs = 30000, resumePdfPath = null, onFieldsChange, onJobDetail, onApplyForm }) {
  ...
  const watcher = startFormWatcher({ page, candidate, resumePdfPath, onFieldsChange, onJobDetail, onApplyForm, jobMap });
