...
const SKILL_LEXICON = [
  ...
  "cuda", "opencv", "qt", "微服务", "容器", "多传感器", "epoll", "socket", "tcp", "http", "线程",
  "并发", "网络编程", "stl", "cmake", "服务器", "操作系统"
];
...
const DIRECTION_LEXICONS = {
  ...
  softdevStrong: ["c++", "c/c++", "epoll", "多线程", "线程池", "网络编程", "嵌入式", "socket", "tcp"],
  softdev: ["linux", "http", "redis", "mysql", "stl", "cmake", "操作系统", "并发", "服务器", "qt", "中间件", "内存", "驱动"],
  agentStrong: ["langgraph", "tool calling", "tool use", "工具调用", "多智能体", "multi-agent", "agent 编排", "智能体架构", "orchestration"]
};
...
export function scoreJobDirection(jdText) {
  ...
  return {
    ...
    softdev: count(DIRECTION_LEXICONS.softdevStrong) * 2 + count(DIRECTION_LEXICONS.softdev),
    agent: count(DIRECTION_LEXICONS.agentStrong) * 2
  };
}

export function pickResumeMode(jdText) {
  const { ai, adas, softdev, agent } = scoreJobDirection(jdText);
  // Agent 编排信号强时优先使用 Agent 基类（LangGraph / 工具调用 / 多智能体等）
  if (agent >= 2 && agent >= ai) return "agent";
  // 软开信号强且显著高于 AI/ADAS 时，使用 C++ 软开基类
  if (softdev >= 3 && softdev >= ai * 2 && softdev >= adas * 2) return "softdev";
...
export function selectBaseResumeFacts(facts, jdText, baseKey = "auto") {
  ...
  const softdevBase = bases.filter((f) => /softdev|cpp/i.test(f.source));
  const agentBase = bases.filter((f) => /agent_system|agent_v2/i.test(f.source));
  const manual = facts.filter((f) => /manual-confirm/i.test(f.source));
  const named = { ai: aiBase, adas: adasBase, softdev: softdevBase, agent: agentBase };
  if (baseKey !== "auto" && named[baseKey]?.length) {
    const extra = manual.filter((m) => !named[baseKey].some((c) => c.id === m.id));
    return { mode: baseKey, chosenBase: baseKey, facts: [...named[baseKey], ...extra] };
  }
  const mode = pickResumeMode(jdText);
  let chosen;
  if (mode === "ai") chosen = aiBase;
  else if (mode === "adas") chosen = adasBase;
  else if (mode === "softdev") chosen = softdevBase;
  else if (mode === "agent") chosen = agentBase;
  else if (mode === "auto") {
    const overlap = (base) => {
    ...
    const candidates = [
      { base: aiBase, score: aiBase.reduce((sum, b) => sum + overlap(b), 0) },
      { base: adasBase, score: adasBase.reduce((sum, b) => sum + overlap(b), 0) },
      { base: softdevBase, score: softdevBase.reduce((sum, b) => sum + overlap(b), 0) },
      { base: agentBase, score: agentBase.reduce((sum, b) => sum + overlap(b), 0) }
    ].filter((c) => c.base.length);
    candidates.sort((a, b) => b.score - a.score);
    chosen = candidates.length ? candidates[0].base : bases;
  } else {
  ...
  const chosenBase =
    chosen === aiBase ? "ai" :
    chosen === adasBase ? "adas" :
    chosen === softdevBase ? "softdev" :
    chosen === agentBase ? "agent" : "ai";
  const extra = manual.filter((m) => !chosen.some((c) => c.id === m.id));
  return { mode, chosenBase, facts: [...chosen, ...extra] };
}
